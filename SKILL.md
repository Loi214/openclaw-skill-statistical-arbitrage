...
description: >
  統計套利（配對交易）專業策略 - 自動識別協整股票對、計算動態對沖比率、執行回測並生成完整報告。
  支持港股（.HK）、美股（無後綴或預設）、A股（.SS/.SZ）等多市場。
  Use when: 用戶說「分析統計套利策略」、「幫我回測AAPL和GOOGL」、「找出協整的股票對」、「配對交易回測」。
  NOT for: 單股票分析、趨勢交易、期貨套利。
metadata:
  openclaw:
    emoji: "📊"
    requires:
...
AI: 掃描美股科技股池...
    Top 3 Pairs:
    1. META-AMZN: p=0.0097, R²=0.90, Score=85.1 ***
    2. AAPL-NVDA: p=0.0131, R²=0.86, Score=79.7 ***
    3. NVDA-AMZN: p=0.0365, R²=0.86, Score=56.6 **
```
